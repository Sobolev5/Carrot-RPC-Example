...
async def sum_a_and_b(incoming_dict: dict) -> dict:
    sprint(incoming_dict, c="yellow", f=1, p=1)
    dct = {}
    #await asyncio.sleep(7) for test timeout
...
async def amqp_router():
    ...
    sprint(f"AMQP:     ready [yes]", c="green", f=1, p=1)
...
@http_router.get("/")
async def main():
    return {"hello": "i m microservice sum (this is my http part)"}
